...
def couple(lst1, lst2):
    ...
    "*** YOUR CODE HERE ***"
    return [[lst1[n],lst2[n]]for n in range(len(lst1))]
...
def closer_city(lat, lon, city1, city2):
    ...
    return get_name(city2) if distance(city1,city0)>=distance(city2,city0) else get_name(city1)
...
def add_trees(t1, t2):
    ...
    if len(branches(t1))==len(branches(t2)):
      return tree(label(t1)+label(t2),[add_trees(branches(t1)[n],branches(t2)[n])for n in range(len(branches(t1)))])
    if len(branches(t1))>len(branches(t2)):
      return tree(label(t1)+label(t2),[add_trees(branches(t1)[n],branches(t2)[n])for n in range(len(branches(t2)))]+[branches(t1)[m]for m in range(len(branches(t2)),len(branches(t1)))])
    if len(branches(t2))>len(branches(t1)):
      return tree(label(t1)+label(t2),[add_trees(branches(t1)[n],branches(t2)[n])for n in range(len(branches(t1)))]+[branches(t2)[m]for m in range(len(branches(t1)),len(branches(t2)))])
