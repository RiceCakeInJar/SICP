...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 4
        env.define(f'{signature}',scheme_eval(expressions.rest.first,env))
        return f'{signature}'
        # END PROBLEM 4
    elif isinstance(signature, Pair) and scheme_symbolp(signature.first):
        # defining a named procedure e.g. (define (f x y) (+ x y))
        # BEGIN PROBLEM 10
        check = signature.rest
        while check:
            if not scheme_symbolp(check.first):
                raise SchemeError
            check = check.rest
        env.define(f'{signature.first}',LambdaProcedure(signature.rest,expressions.rest,env))
        return f'{signature.first}'
        # END PROBLEM 10
    else:
