def count(t, n, x):
    ...
    2
    """
    return [next(t)for x in range(n)].count(x)
def repeated(t, k):
    ...
    assert k >= 1
    tr1 = k
    def check_and_return(ans,k):
        if k==1:
            return ans
        tr=next(t)
        if ans==tr:
            return check_and_return(ans,k-1)
        else:
            return check_and_return(tr,tr1)
    return check_and_return(next(t),k)
