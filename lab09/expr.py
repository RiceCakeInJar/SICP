...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        """
        "*** YOUR CODE HERE ***"
        _operator = self.operator.eval(env)
        _operands = [x.eval(env) for x in self.operands]
        return _operator.apply(_operands)
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        "*** YOUR CODE HERE ***"
        _env = self.parent.copy()
        _args = arguments[:]
        for x in self.parameters:
            _env[f'{x}'] = _args[0]
            _args.pop(0)
        return self.body.eval(_env)
