...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            copy_ant = place.ant
            place.ant = self
            self.store_ant(copy_ant)
        elif self.is_container == False and place.ant.is_container == True and place.ant.ant_contained == None:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        def help(some_place,distance):
            if some_place.is_hive == True:
                return
            if some_place.bees!=[] and self.min_range<=distance<=self.max_range:
                return random_bee(some_place.bees)
            return help(some_place.entrance,distance+1)
        return help(self.place,0)
        # END PROBLEM 3 and 4
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.chew_countdown==0:
            if self.place.bees!=[]:
                unlucy_bee=random_bee(self.place.bees)
                unlucy_bee.reduce_health(unlucy_bee.health)
    ...
    def action(self, gamestate):
        ...
        while True:
            if buffer.exit!=None:
                ...
                if buffer.ant != None:
                    buffer.ant.buff()
                    if buffer.ant.is_container==True:
                        if buffer.ant.ant_contained != None:
                            buffer.ant.ant_contained.buff()
            elif buffer.exit==None:
                break
        # END PROBLEM 12
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        elif self.scare_time !=0 and self.place.entrance.is_hive==False:
        ...
        if self.slow_time>0:
            ...
            if gamestate.time%2==0:
                ...
                if self.place!=None:
                    if destination!=self.place.exit and self.scare_time>0:
                        self.scare_time-=1
            else:
                pass
        else:
