...
def pingpong(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def bounce(ans,go,where):
        if where==n:
            return ans
        elif number_of_six(where)>=1 or where%6==0:
            return bounce(ans-go,-go,where+1)
        else:
            return bounce(ans+go,go,where+1)
    return bounce(1,1,1)
...
def count_change(total, next_money):
    ...
    "*** YOUR CODE HERE ***"
    def change(need,now):
        if now>total or need<0:
            return 0
        elif need==0:
            return 1
        else:
            if next_money(now)!=None:
                return change(need-now,now)+change(need,next_money(now))
            else:
                return change(need-now,now)
...
def multiadder(n):
    ...
    def adder(n,box):
        if n==0:
            ...
            return lambda x:adder(n-1,box+x)
    return adder(n,0)
